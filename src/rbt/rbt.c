...
// functions
static int RBT_malloc(rbt **root, void *key, tree_operations *ops,
                      rbt *parent) {
  ...
  (*root)->operations = ops;
  (*root)->parent = parent;
  (*root)->operations->ref_number += 1;
  return 0;
}

static void reset_parent_after_rotation(rbt *parent, rbt *child,
...
static int double_rotation(rbt *grandparent, rbt *parent, int index_pr_ch,
                           rbt *child, enum ROTATION_DIRECTION first_rotation,
                           enum ROTATION_DIRECTION second_rotation) {
  if (parent->children[index_pr_ch] != NULL &&
      parent->children[index_pr_ch] == child) {
...
static int double_black_brother_red_delete(rbt *brother, rbt *parent,
                                           int which_child) {
  ...
  if (brother->children[!which_child] == NULL &&
      brother->children[which_child]->colour == RED) {
  ...
  reset_parent_after_rotation(parent, brother->children[which_child],
                              second_rotation_to_apply);
...
static int double_black_brother_black_delete(rbt *brother, rbt *parent,
                                             int which_child) {
...
int RBT_delete(rbt **root, void *key) {
  ...
  if (root_conv->children[0] != NULL && root_conv->children[1] == NULL &&
      root_conv->children[0]->colour == RED) {
  ...
  if (root_conv->children[0] == NULL && root_conv->children[1] != NULL &&
      root_conv->children[1]->colour == RED) {
...
rbt *get_right_child(rbt *root) { return root->children[1]; }

rbt *get_left_child(rbt *root) { return root->children[0]; }
